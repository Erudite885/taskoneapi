...
const FormCustomQuestion = ({
  ...
  const formDropdownChoices: React.ReactNode = (() => {
    const renderChoice = (choice: string, index: number) => {
      return (
        <div
          className="grid grid-flow-col w-full items-center grid-cols-[max-content_auto] gap-[7px]"
          key={index}
        >
          <FormFieldLayout
            label={<p className="invisible">.</p>}
            content={<img src={MenuH} alt="menu icon" />}
          />
          <FormInput
            label="Choice"
            placeholder="Type here"
  ...
  switch (formQuestion.type) {
    ...
    case "Number":
    case "FileUpload": {
      formElements.push(formInputQuestion);
      break;
    }
    case "YesNo": {
      formElements.push(formInputQuestion, formDisqualify);
      break;
    }
    case "MultipleChoice":
    case "Dropdown": {
      formElements.push(formInputQuestion, formDropdownChoices);

      if (formQuestion.type === "MultipleChoice") {
        formElements.push(formMaxChoice);
      }
      break;
    }
    case "Video": {
      formElements.push(formTextareaQuestion, formVideoDuration);
      break;
    }
    default: {
      break;
...
export default FormCustomQuestion;
